...
pub fn set_fill_animation_3d(obj_3d: ThreeDObject, target_fill: (f64, f64, f64, f64), t: f64) -> ThreeDObject {
    let mut obj_3d = obj_3d;
    match obj_3d.get_fill() {
        GradientImageOrColor::Color(color) => {
            let color = (color.red, color.green, color.blue, color.alpha);
            let new_color = interpolate_color(color, target_fill, t);
            obj_3d = obj_3d.set_fill(GradientImageOrColor::Color(Color {
                red: new_color.0,
                green: new_color.1,
                blue: new_color.2,
                alpha: new_color.3,
    ...
    obj_3d.subobjects = obj_3d.subobjects.iter().map(|subobj| {
        return set_fill_animation_3d(subobj.clone(), target_fill, t);
    }).collect();
    return obj_3d;
}