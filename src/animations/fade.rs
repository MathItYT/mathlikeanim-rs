...
pub fn fade_in(scale_factor: f64, shift: (f64, f64)) -> impl Fn(VectorFeatures, f64) -> VectorFeatures {
    let animation_func = move |vec_obj: VectorFeatures, t: f64| -> VectorFeatures {
        ...
        result.subobjects = result.subobjects.iter().map(|subobj| {
            return subobj.clone().set_fill_opacity(interpolate(0.0, subobj.get_fill_opacity(), t), false)
                .set_stroke_opacity(interpolate(0.0, subobj.get_stroke_opacity(), t), false);
        }).collect();
        return result;
    };
...
pub fn fade_out(scale_factor: f64, shift: (f64, f64)) -> impl Fn(VectorFeatures, f64) -> VectorFeatures {
    let animation_func = move |vec_obj: VectorFeatures, t: f64| -> VectorFeatures {
        ...
        result.subobjects = result.subobjects.iter().map(|subobj| {
            return subobj.clone().set_fill_opacity(interpolate(subobj.get_fill_opacity(), 0.0, t), false)
                .set_stroke_opacity(interpolate(subobj.get_stroke_opacity(), 0.0, t), false);
        }).collect();
        return result;
    };
    return animation_func;
